...
-- Ver los datos de la tabla "tarjetas"
SELECT * FROM tarjetas;

-- Sub consultas
-- Se pueden usar consultas usadas anteriormente para poder realizar consultas dentro de los valores que obtuvimos anteriormente.
SELECT monto, cliente
FROM tarjetas
-- cliente en la tabla tarjeta contiene los ids de los clientes
WHERE cliente in (
-- Consulta usada en la línea 77
	SELECT id_cliente
	FROM clientes
	WHERE (edad BETWEEN 20 AND 50) AND (estrato=1)
);
-- Se usan llaves foráneas (FOREIGN KEYs) para hacer sub consultas para que las subconsultas tengan mas coherencia.
...
-- Implementación del COUNT
-- Conteo de los valores que la consulta va a regresar.
SELECT COUNT(nombre) AS conteo_mensual
FROM clientes
WHERE edad=(
	SELECT max(edad)
	FROM clientes
...
-- Variable categórica
	-- Variable no-númerica, no representa una magnitud con respect a las otras.
	-- No tiene relevancia numérica.
	-- Permiten segmentar por cualidades.
    
-- GROUP BY
-- Regresa una tabla con los registros que cumplan el filtro del GROUP BY.
SELECT pais, MAX(ingresos) AS ingresos, MIN(edad) AS edad
FROM clientes
GROUP BY pais;

-- Sentencia para ordenar
	-- Se usa DESC para ordenar en descendencia de datos.
	-- Se usa ASC para ordenar en ascendencia de datos.
SELECT edad
FROM clientes
ORDER BY edad DESC;

-- Ejemplo
SELECT pais, MAX(ingresos), MIN(edad)
FROM clientes
GROUP BY pais
-- Primero ordena por ingresos, pero si encuentra los mismos valores va a ordenar por la edad 
ORDER BY MAX(ingresos), MIN(edad) ASC;
